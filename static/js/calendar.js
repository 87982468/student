...
function Calendar(beginYear, endYear, language, patternDelimiter, date2StringPattern, string2DatePattern) {
	...
	this.colors = {"bg_cur_day":"#ff9900","bg_over":"#edf1f2","bg_out":"#FFCC00","bg_selected":"#567076"}
};
...
Calendar.prototype.bindData = function () {
	...
	for(var i = 0; i < tds.length; i++) {
		...
		if (dateArray[i]){
			...
			// add new lines by tony
			var selectedday = this.date;
			if (selectedday.getFullYear() == calendar.date.getFullYear()) {
				if (selectedday.getMonth() == calendar.date.getMonth()) {
					if (selectedday.getDate() == dateArray[i]) {
						tds[i].style.backgroundColor = calendar.colors["bg_selected"];
						tds[i].onmouseover = function () {this.style.backgroundColor = calendar.colors["bg_out"];}
						tds[i].onmouseout  = function () {this.style.backgroundColor = calendar.colors["bg_selected"];}
					}
				}
			}
			// end add
...
String.prototype.toDate = function(delimiter, pattern) {
	delimiter = delimiter || "-";
	pattern = pattern || "ymd";
	var dateobj = this.replace(/\-/g, delimiter);
	var a = dateobj.split(delimiter);
...
var divs = [];
divs[divs.length] = '<div id="__calendarPanel" style="position:absolute;z-index:99999;display:none;background:#b4cdda;border:1px solid #dfe5f7;width:200px;height:216px;box-shadow:0px 1px 6px #247;">';
...
cis[cis.length] ='<!--';
cis[cis.length] ='body {font:12px/15px "宋体";margin:0px;text-align:center;background:#b4cdda;}';
cis[cis.length] ='form {margin:0px;}';
cis[cis.length] ='select {font-size:12px;background-color:#edf1f2;}';
cis[cis.length] ='table {border:0px;background:#b4cdda}';
cis[cis.length] ='th {font-size:12px;font-weight:normal;background:#b4cdda;}';
cis[cis.length] ='th.theader {font-weight:normal;background-color:#567076;color:#FFFFFF;width:24px;}';
cis[cis.length] ='select.year {width:64px;}';
cis[cis.length] ='select.month {width:60px;}';
cis[cis.length] ='td {font-size:12px;height:20px;padding:0px;text-align:center;cursor:pointer;}';
cis[cis.length] ='td.sat {color:#567;background-color:#c4d1d4;}';
cis[cis.length] ='td.sun {color:#567;background-color:#c4d1d4;}';
cis[cis.length] ='td.normal {background-color:#edf1f2;}';
cis[cis.length] ='input.l,input.r,input.b {border-top:1px solid #fff;border-left:1px solid #fff;border-right:1px solid #6b8c9d;border-bottom:1px solid #6b8c9d;background:#8babbb;color:#FFF;width:20px;height:20px;}';
cis[cis.length] ='input.b {width:100%;}';
cis[cis.length] ='-->';
